...
$\blacksquare$ 尼斯特伦法对于 $n = 3$ 解积分方程 $\varphi \left( x\right)  = \cos {\pi x} + \frac{x}{{x}^{2} + {\pi }^{2}}\left( {{\mathrm{e}}^{x} + 1}\right)  +$${\int }_{0}^{1}{\mathrm{e}}^{xy}\varphi \left( y\right) \mathrm{d}y$
