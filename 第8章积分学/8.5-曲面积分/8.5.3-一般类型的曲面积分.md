...
$\blacksquare$ 球面公式 ${x}^{2} + {y}^{2} + {z}^{2} = {R}^{2}$ ,要想计算球体体积 $V$ ,需要利用球坐标 $x =$ $R\sin \vartheta \cos \varphi , y = R\sin \vartheta \sin \varphi , z = R\cos \vartheta \left( {0 \leq  \vartheta  \leq  \pi ,0 \leq  \varphi  \leq  {2\pi }}\right)$ 以及如 (8.160a) 中那样的雅可比行列式
...
$\blacksquare$ 分方程 $- {y}^{\prime }\sin x + y\cos x = 1$ 有通解 $y = \cos x + c\sin x$ . 对于条件 $c = 0$ ,得到特解 $y = \cos x$ .
...
$\blacksquare$ 点固定和均匀负载的细棒形成的曲线由具有边界条件 $y\left( 0\right)  = 0, y\left( 1\right)  = 0$ 的微分方程 ${y}^{\prime \prime } = x - {x}^{2}\left( {0 \leq  x \leq  1}\right)$ 所描述. 其解为 $y = \frac{{x}^{3}}{6} - \frac{{x}^{4}}{12} - \frac{x}{12}$ .
