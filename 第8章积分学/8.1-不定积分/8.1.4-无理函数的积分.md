...
$\blacksquare$ 过椭圆弧长的计算,可得第二类完全椭圆积分为离心率 $e$ 的函数 (参见第 668 页 8.2.2.2,2.). 当 $a = {1.5}, b = 1$ 时, $e = {0.74}$ . 又 $e = k = {0.74}$ ,由 1425 页表 21.9.3,有 $\sin \alpha  = {0.74}$ ,即 $\alpha  \approx  {48}^{ \circ  }$ ,且 $E\left( {k,\frac{\pi }{2}}\right)  = E\left( {0.74}\right)  = {1.3238}$ . 由此 $U = {4aE}\left( {0.74}\right)  \approx  {4aE}\left( {\alpha  \approx  {48}^{ \circ  }}\right)  = 4 \cdot  {1.3238a} \approx  {7.94}$ . 利用数值积分可进一步近似成 7.932711.
