...
$\blacksquare$ 定定积分 $I = {\int }_{-{1000}}^{1000}{\mathrm{e}}^{-{x}^{2}}\mathrm{\;d}x$ . >> format long; [I, fwerte]=quad(@(x)(exp(-x.^2)),-1000,1000,1e-10) $I = {1.77245385094233}$ fwerte $= {585}$
...
$\blacksquare$ 龙格-库塔法 (参见第 1260 页 19.4.1.2) 求解区间 $\left\lbrack  {0,1}\right\rbrack$ 上的问题 ${y}^{\prime } = \frac{1}{4}\left( {{x}^{2} + }\right.$ $\left. {y}^{2}\right) , y\left( 0\right)  = 0$ ,输入:
...
$\blacksquare$  $\operatorname{In}\left\lbrack  3\right\rbrack   \mathrel{\text{:=}} \operatorname{Plot}\left\lbrack  {\operatorname{Interpolation}\left\lbrack  l\right\rbrack  \left\lbrack  x\right\rbrack  ,\{ x,1,{10}\} }\right\rbrack$ 得图 19.21(b). 显然 Mathemat-ica 给出了精确对应于数据表的结果.
...
$\blacksquare$ 函数 $y = f\left( x\right)  = \sqrt{x} + \ln x$ 的数值表. 首先由箭头算子定义函数:
...
$\blacksquare$ 算定积分 ${\int }_{-2}^{2}{\mathrm{e}}^{-{x}^{3}}\mathrm{\;d}x$ . 因为原函数未知,对于积分命令,得到如下答案:
